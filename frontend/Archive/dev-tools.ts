...
const getApiBaseUrl = (): string => {
	...
	if (typeof global !== 'undefined' && global.__DEV__) {
		try {
			...
			console.log(
				'[HealthService] Could not extract IP from Metro URL:',
				error
			);
		}
	}
	...
	console.warn(
		'[HealthService] Using localhost fallback - mobile devices may not connect'
	);
...
class HealthMonitorClient {
	...
	async checkBackendHealth(): Promise<HealthCheckResult> {
		try {
			const healthData = await this.makeRequest<BackendHealthResponse>(
				'/health'
			);
