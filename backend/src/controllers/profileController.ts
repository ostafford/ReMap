	...
	async (req: Request, res: Response) => {
		...
		try {
			if (file) {
				...
				// Generate a more robust filename with timestamp and random suffix
				// We can change this to something proper if we want to
				const timestamp = Date.now();
				const randomSuffix = Math.random().toString(36).substring(2, 8);
				const fileExtension = file.mimetype.split('/')[1] || 'jpg';
				const fileName = `${user_name}_${timestamp}_${randomSuffix}.${fileExtension}`;
