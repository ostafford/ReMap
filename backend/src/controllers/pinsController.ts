...
export const createPin = [
	...
	async (req: Request, res: Response) => {
		...
		try {
			// Audio upload logic
			if (audioFile) {
				// Check if the file is an audio (starts with "audio/")
		...
		// Validate and parse new fields
		let parsedSocialCircleIds: string[] = [];
		if (social_circle_ids) {
			try {
				parsedSocialCircleIds = JSON.parse(social_circle_ids);
				if (!Array.isArray(parsedSocialCircleIds)) {
					console.log('Social circle IDs must be an array');
					res.status(400).json('Social circle IDs must be an array');
					return;
				}
			} catch (err) {
				console.log('Invalid social circle IDs format');
				res.status(400).json('Invalid social circle IDs format');
				return;
			}
		}

		// Validate visibility
		const validVisibilities = ['public', 'social', 'private'];
		if (visibility && !validVisibilities.includes(visibility)) {
			console.log('Visibility must be public, social, or private');
			res.status(400).json(
				'Visibility must be public, social, or private'
			);
			return;
		}

		// Privacy logic validation
		if (visibility === 'social' && parsedSocialCircleIds.length === 0) {
			console.log(
				'Social pins must have at least one social circle selected'
			);
			res.status(400).json(
				'Social pins must have at least one social circle selected'
			);
		...
		try {
			...
			const { data, error } = await supabase
				...
				.insert({
					...
					social_circle_ids: parsedSocialCircleIds,
